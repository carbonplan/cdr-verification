...
def get_component_sheet(*, google_doc_id: str) -> pd.DataFrame:
    ...
    cdf['uncertainty_type'] = cdf.uncertainty_type.str.replace(' ', '').apply(
        lambda x: x.split(',')
    )
...
def get_all_sheets_in_doc(gsheet_doc_name: str) -> dict:
    ...
    sheet_id_name_dict = {
        i.get('properties', {}).get('title', 0): i.get('properties', {}).get('sheetId', 0)
        for i in sheets
    }
...
def sheet_data_to_dataframe(data_list: list) -> pd.DataFrame:
    ...
    return (
        pd.DataFrame(data_list[10::], columns=data_list[9])[pathways_data_columns]
        .replace('', np.nan)
        .dropna(how='all')
    )
...
def contributors_df(*, google_doc_id: str) -> pd.DataFrame:
    ...
    data_list = get_data_values_by_sheet_name(
        google_doc_id=google_doc_id, sheet_name='Contributors'
    )
    return pd.DataFrame(data_list[1::], columns=data_list[0])
...
def sheet_data_to_metadata(sheet_data: list) -> dict:
    ...
    VCL = (
        list(tuple(sheet_data[3][1].replace(' ', '').split(','))) if len(sheet_data[3]) > 1 else ''
    )
...
def write_legend_to_json(df: pd.DataFrame):
    ...
    with open('data/legend.json', 'w') as fp:
...
def contributors_to_json(df: pd.DataFrame):
    ...
    for _, subset in df.iterrows():
        ...
        spathway = dropped.drop(
            ['type', 'name', 'affiliation', 'initial', 'notes'], errors='ignore'
        )
    ...
    with open('data/contributors.json', 'w') as fp:
...
def write_metadata_to_json(*, pathway: str, metadata_dict: dict, contributor_df: pd.DataFrame):
    ...
    contributor_df_subset = contributor_df[~contributor_df[pathway_id].isnull()][
        ['type', 'name', 'affiliation', 'notes', pathway_id]
    ]
    ...
    contributor_df_subset['version'] = (
        contributor_df_subset['version'].str.replace(' ', '').apply(lambda x: x.split(','))
    )
...
def process_sheet(gsheet_doc_name: str, worksheet_name: str):
    data_list = get_data_values_by_sheet_name(
        google_doc_id=google_doc_id, sheet_name=worksheet_name
    )
...
def write_pathways_to_json(avail_pathways: list, google_doc_id: str):
    ...
    for pathway in avail_pathways:
        ...
        write_metadata_to_json(
            pathway=pathway,
            metadata_dict=process_sheet_dict['metadata_dict'],
